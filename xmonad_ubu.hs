...
-- imports for netbeans keyboard error
...
main = do
     ...
     xmonad $ defaultConfig {
            ...
            where
                ...
                bg    = "'#2c2c32'"
                fn    = "'-*-terminus-*-*-*-*-12-*-*-*-*-*-*-u'"
                flags = "-e '' -w 1280 -ta l -fg " ++ fg ++ " -bg " ++ bg ++ " -fn " ++ fn

keys' = [((mod4Mask .|. shiftMask, xK_z), spawn "xlock -mode matrix"),
         ((controlMask, xK_Print), spawn "scrot -s"),
         ((0, xK_Print), spawn "scrot"),
         ((controlMask, xK_F3), spawn "setxkbmap mn && mymodmap.sh" ),
         ((controlMask, xK_F9), spawn "setxkbmap mn && mymodmap.sh" ),
         ((mod4Mask, xK_F9), spawn "setxkbmap mn && mymodmap.sh" ),
         ((controlMask, xK_F2), spawn "setxkbmap us && mymodmap.sh" ),
         ((controlMask, xK_F8), spawn "setxkbmap us && mymodmap.sh" ),
         ((mod4Mask, xK_F8), spawn "setxkbmap us && mymodmap.sh" ),
         ((controlMask, xK_F4), spawn "setxkbmap de && mymodmap.sh" ),
         ((mod4Mask, xK_u ), spawn "uxterm"),
         ((mod4Mask, xK_F7), moveTo Next EmptyWS),
         ((mod4Mask, xK_F11), spawn "setxkbmap mn && mymodmap.sh"  >> toggleWS),
         ((mod4Mask, xK_F10), spawn "setxkbmap us && mymodmap.sh"  >> toggleWS)
        ]
